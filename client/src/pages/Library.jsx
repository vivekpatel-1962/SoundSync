...
export default function Library() {
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={onCreate} className="flex gap-3 max-w-md">
        <Input
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {(() => {
          ...
          if (safeEntries.length === 0) {
            return (
              <div className="text-slate-400">No playlists yet. Use the mini-player ＋ or ♥ to add songs.</div>
            );
          }
          return safeEntries.map(([name, list]) => {
            ...
            return (
              <Card key={name} className="hover:shadow-xl transition-all duration-300 overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 w-full">
                        <div className="min-w-0">
                          <div className="font-semibold text-white truncate whitespace-nowrap w-full pr-2" title={name}>{name}</div>
                          <div className="text-sm text-slate-400">{(list || []).length} songs</div>
                        </div>
                      ...
                      {(list || []).length === 0 && (
                        <div className="mt-4 text-sm text-slate-400">No songs yet. Use ＋ in mini-player or "Add current" to add songs.</div>
                      )}
                    </div>
                  </div>
                </CardContent>
